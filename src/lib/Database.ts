...
class Database {
    ...
    flush () {
      ...
      // TO FIX fs usage
    //   if (this.debugMode) { fs.writeFileSync('db-test.json', JSON.stringify(this)) } else { fs.writeFileSync('db.json', JSON.stringify(this)) }
    }
    ...
    load () {
      ...
      const str = '{}'
      // TO FIX fs usage
      //   if (this.debugMode) { str = fs.readFileSync('db-test.json', { encoding: 'utf-8' }) } else { str = fs.readFileSync('db.json', { encoding: 'utf-8' }) }
